...
public class Zipper {
  ...
  public String encode(String plainText) {
	  ...
	  for (int i = 0; i < plainText.length(); i++) {
		  string.append(book.encodeChar(plainText.charAt(i)));
	  }
  ...
  public String decode(String bits) {
	  ...
	  for (int i = 0; i < bits.length(); i++) {
		 string.append(Character.toString(bits.charAt(i)));
		 if (this.ht.decodeChar(string.toString()) != '\0') {
			 result.append(Character.toString(this.ht.decodeChar(string.toString())));
